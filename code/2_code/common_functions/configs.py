...
class Configs:
    ...
    def get_data_config_str(data_config):
        ...
        if data_config['filter_RDT']:
            s += 'RDT_thres=(' + str(data_config['R_threshold']) + ',' \
                 + str(data_config['D_threshold']) + ',' \
                 + str(data_config['T_threshold']) + ')' + ','
            s += 'RDT_bi=(' + str(int(data_config['binary_R'])) + ',' \
                 + str(int(data_config['binary_D'])) + ',' \
                 + str(int(data_config['binary_T'])) + ')' + ','

        if data_config['extra_process']:
            if data_config['new_d_proportion'] > 0:
                s += 'new_d=' + str(data_config['new_d_proportion']) + ','
            if data_config['new_t_proportion'] > 0:
                s += 'new_t=' + str(data_config['new_t_proportion']) + ','
            if data_config['d_sparsity'] > 0:
                s += 'ds=' + str(data_config['d_sparsity']) + ','
            if data_config['t_sparsity'] > 0:
                s += 'ts=' + str(data_config['t_sparsity']) + ','
            if data_config['r_sparsity'] > 0:
                s += 'rs=' + str(data_config['r_sparsity']) + ','
            if data_config['r_noise_add'] > 0:
                s += 'r_add=' + str(data_config['r_noise_add']) + ','
            if data_config['r_noise_del'] > 0:
                s += 'r_del=' + str(data_config['r_noise_del']) + ','
            if data_config['d_noise_add'] > 0:
                s += 'd_add=' + str(data_config['d_noise_add']) + ','
            if data_config['d_noise_del'] > 0:
                s += 'd_del=' + str(data_config['d_noise_del']) + ','
            if data_config['t_noise_add'] > 0:
                s += 't_add=' + str(data_config['t_noise_add']) + ','
            if data_config['t_noise_del'] > 0:
                s += 't_del=' + str(data_config['t_noise_del']) + ','
    ...
    def get_model_config_str(model_config):
        ...
        if model_config['method'] == 'BGCCL':
            s += 'lalg=' + str(model_config['loss_LP_alg']) + ','
