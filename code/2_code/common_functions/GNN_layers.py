...
class GCN_Layer(Layer):
    def __init__(self, units=200, use_bias=False, activation=tf.nn.relu,
                 l2_normalize=True, l2_reg=0, seed=0, **kwargs):
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.l2_normalize:
            return tf.nn.l2_normalize(self.activation(A_norm @ H @ self.W), axis=1)
        else:
            return self.activation(A_norm @ H @ self.W)
...
class GAT_Layer(Layer):
    def __init__(self, attention_num=8, units=8,
                 activation=(tf.nn.leaky_relu, tf.nn.elu), l2_normalize=True, l2_reg=0.0005,
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.l2_normalize:
            return tf.nn.l2_normalize(H_out, axis=1)
        else:
            return H_out
...
class LightGCN_Layer(Layer):
    def __init__(self, units=200, l2_normalize=True, seed=0, **kwargs):
...
class GIN_Layer(Layer):
    def __init__(self, units=200, use_bias=True, activation=tf.nn.relu,
                 l2_normalize=True, l2_reg=0, seed=0, **kwargs):
