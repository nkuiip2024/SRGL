...
if dataset in {'DiseaseGene', 'movielens_1m'}:
    batch_sampler = 'Grid'
else:
    batch_sampler = 'None'
use_d_features = False  # Do not change it!
use_t_features = False  # Do not change it!
use_D = False  # Do not change it!
use_T = False  # Do not change it!
levels = 4
units = 200
optimizer = 'rmsprop'
loss_mode = 'R'  # Do not change it!
epochs = 500
metric_group_idx = 2
cv_metric = 'auc_train'
...
data_config = {
    'n_folds': 10,
    'val_prop': 0.0,  # Useful only if n_folds==1
    'test_prop': 0.1,  # Useful only if n_folds==1
    'balance_01': True,
    ...
    'extra_process': False,  # Do not change it
}
batch_config = {
...
if method == 'SRGL':
    ...
    model_config = dict(model_config, **SRGL_config)
...
if method == 'SRGL':
    paras_groups = (
        (0,),
        tuple(np.r_[0, 10.0 ** np.arange(-4, 5)]),
        (1,),
        (1,)
    )
